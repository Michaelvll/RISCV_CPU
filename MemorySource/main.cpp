...
int main(int argc, char *argv[]) {
  ...
  options.add_options()("help,h", "Display this information")(
      "memory", value<std::string>()->value_name("file"),
      "[in] Initial Memory File")(
      "memory-size", value<int>()->value_name("size"),
      "Memory Size")("io-input", value<std::string>()->value_name("file"),
                     "[in] I/O Input File")(
      "io-output", value<std::string>()->value_name("file"),
      "[out] I/O Output File")("time-file",
                               value<std::string>()->value_name("file"),
                               "[out] The running time")(
      "com-port", value<std::string>()->value_name("port"),
      "The serial port connected to the fpga")("verbose",
  ...
  Environment env(vm["com-port"].as<std::string>(), memSize,
                  vm.count("verbose"));
  ...
  if (vm.count("io-input")) {
    fioIn.open(vm["io-input"].as<std::string>(),
               std::ios::binary | std::ios::in);
    ioIn = &fioIn;
  } else
    ioIn = &ss1;
  if (vm.count("io-output")) {
    fioOut.open(vm["io-output"].as<std::string>(),
