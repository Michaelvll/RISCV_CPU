...
*
*               uC/CPU is provided in source form to registered licensees ONLY.
*It is illegal to distribute this source code to any third party unless you
*receive written permission by an authorized Micrium
*representative.  Knowledge of the source code may NOT be used to
...
*notes and more information at doc.micrium.com. You can contact us at
*www.micrium.com.
*********************************************************************************************************
*/
...
*                                               MODULE
*
* Note(s) : (1) This CPU header file is protected from multiple pre-processor
*inclusion through use of the  CPU module present pre-processor macro
*definition.
*********************************************************************************************************
*/
...
*
*                       where
*                               <Your Product Application>      directory path
*for Your Product's Application <CPU-Compiler Directory>        directory path
*for common   CPU-compiler software <cpu>
*directory name for specific CPU <compiler>
*directory name for specific compiler
*
*           (2) Compiler MUST be configured to include as additional include
*path directories :
*
*               (a) '\<Your Product Application>\' directory See Note #1a
*
*               (b) (1) '\<CPU-Compiler Directory>\'                  directory
*See Note #1b1 (2) '\<CPU-Compiler Directory>\<cpu>\<compiler>\' directory
*See Note #1b2
*
*           (3) Since NO custom library modules are included, 'cpu.h' may ONLY
*use configurations from CPU configuration file 'cpu_cfg.h' that do NOT
*reference any custom library definitions.
*
*               In other words, 'cpu.h' may use 'cpu_cfg.h' configurations that
*are #define'd to numeric constants or to NULL (i.e. NULL-valued #define's); but
*may NOT use configurations to custom library #define's (e.g.
...
*
* Note(s) : (1) Configure standard data types according to
*CPU-/compiler-specifications.
*
*           (2) (a) (1) 'CPU_FNCT_VOID' data type defined to replace the
*commonly-used function pointer data type of a pointer to a function which
*returns void & has no arguments.
*
...
*                           FnctName();
*
*               (b) (1) 'CPU_FNCT_PTR'  data type defined to replace the
*commonly-used function pointer data type of a pointer to a function which
*returns void & has a single void pointer argument.
*
*                   (2) Example function pointer usage :
*
...
*
* Note(s) : (1) Configure CPU_CFG_ADDR_SIZE, CPU_CFG_DATA_SIZE, &
*CPU_CFG_DATA_SIZE_MAX with CPU's &/or compiler's word sizes :
*
...
*CPU_ENDIAN_TYPE_LITTLE      Little-endian word order (CPU words'
*least significant octet @ lowest memory address)
*********************************************************************************************************
*/
/* Define  CPU         word sizes (see Note #1) :       */
#define CPU_CFG_ADDR_SIZE                                                      \
  CPU_WORD_SIZE_32 /* Defines CPU address word size  (in octets).          */
#define CPU_CFG_DATA_SIZE                                                      \
  CPU_WORD_SIZE_32 /* Defines CPU data    word size  (in octets).          */
#define CPU_CFG_DATA_SIZE_MAX                                                  \
  CPU_WORD_SIZE_64 /* Defines CPU maximum word size  (in octets).          */

#define CPU_CFG_ENDIAN_TYPE                                                    \
  CPU_ENDIAN_TYPE_LITTLE /* Defines CPU data    word-memory order (see Note    \
                            #2). */
...
*
* Note(s) : (1) Configure CPU_CFG_STK_GROWTH in 'cpu.h' with CPU's stack growth
*order :
*
*               (a) CPU_STK_GROWTH_LO_TO_HI     CPU stack pointer increments to
*the next higher  stack memory address after data is pushed onto the stack (b)
*CPU_STK_GROWTH_HI_TO_LO     CPU stack pointer decrements to the
*next lower   stack memory address after data is pushed onto the stack
*
*           (2) Configure CPU_CFG_STK_ALIGN_BYTES with the highest minimum
*alignement required for cpu stacks.
*********************************************************************************************************
*/

#define CPU_CFG_STK_GROWTH                                                     \
  CPU_STK_GROWTH_HI_TO_LO /* Defines CPU stack growth order (see Note #1). */

#define CPU_CFG_STK_ALIGN_BYTES                                                \
  (sizeof(CPU_ALIGN)) /* Defines CPU stack alignment in bytes. (see Note #2).  \
                       */
...
*
* Note(s) : (1) Configure CPU_CFG_CRITICAL_METHOD with CPU's/compiler's critical
*section method :
*
*                                                       Enter/Exit critical
*sections by ...
*
*                   CPU_CRITICAL_METHOD_INT_DIS_EN      Disable/Enable
*interrupts CPU_CRITICAL_METHOD_STATUS_STK      Push/Pop       interrupt status
*onto stack CPU_CRITICAL_METHOD_STATUS_LOCAL    Save/Restore
*interrupt status to local variable
*
*               (a) CPU_CRITICAL_METHOD_INT_DIS_EN  is NOT a preferred method
*since it does NOT support multiple levels of interrupts.  However, with some
*CPUs/compilers, this is the only available method.
*
*               (b) CPU_CRITICAL_METHOD_STATUS_STK    is one preferred method
*since it supports multiple levels of interrupts.  However, this method assumes
*that the compiler provides C-level
...
*
*               (c) CPU_CRITICAL_METHOD_STATUS_LOCAL  is one preferred method
*since it supports multiple levels of interrupts.  However, this method assumes
*that the compiler provides C-level
*                   &/or assembly-level functionality for the following :
*
...
*If the compiler does NOT allow inline assembly in C source files, critical
*section macro's MUST call an assembly subroutine defined in a
*'cpu_a.asm' file located in the following software directory :
*
*                   \<CPU-Compiler Directory>\<cpu>\<compiler>\
*
*                       where
*                               <CPU-Compiler Directory>    directory path for
*common   CPU-compiler software <cpu>                       directory name for
*specific CPU <compiler>
*directory name for specific compiler
*
*           (3) (a) To save/restore interrupt status, a local variable 'cpu_sr'
*of type 'CPU_SR' MAY need to be declared (e.g. if
*'CPU_CRITICAL_METHOD_STATUS_LOCAL' method is configured).
*
*                   (1) 'cpu_sr' local variable SHOULD be declared via the
*CPU_SR_ALLOC() macro which, if used, MUST be declared following ALL other local
*variables.
*
*                        Example :
*
...
*                               CPU_INT16U  val_16;
*                               CPU_INT32U  val_32;
*                               CPU_SR_ALLOC();         MUST be declared after
*ALL other local variables
*                                   :
*                                   :
*                           }
*
*               (b) Configure 'CPU_SR' data type with the appropriate-sized CPU
...
typedef CPU_INT32U
    CPU_SR; /* Defines   CPU status register size (see Note #3b).   */
...
#define CPU_INT_DIS()                                                          \
  do {                                                                         \
    cpu_sr = CPU_SR_Save();                                                    \
  } while (0) /* Save    CPU status word & disable interrupts.*/
#define CPU_INT_EN()                                                           \
  do {                                                                         \
    CPU_SR_Restore(cpu_sr);                                                    \
  } while (0) /* Restore CPU status word.                     */
#endif
